...
function convertToDate(cadena){
    debugger;

    var fecha = new Date();
    cadfecha =  []; 
    if(cadena.includes('-')){
            cadfecha = cadena.split('-');
    }
    if(cadena.includes('/')){
            cadfecha = cadena.split('-');
    }

    cadfecha[0]=parseInt(cadfecha[0]);
    cadfecha[1]=parseInt(cadfecha[1]);
    cadfecha[2]=parseInt(cadfecha[2]);
    cadfecha[0]=cadfecha[0].toString();
    cadfecha[1]=cadfecha[1].toString();
    cadfecha[2]=cadfecha[2].toString();

    if(cadfecha[0].length==4){
          fecha=new Date(cadfecha[0],(cadfecha[1]-1),cadfecha[2]);
    }
    else{
        fecha=new Date(cadfecha[2],(cadfecha[1]-1),cadfecha[0]);
    }
    debugger;
    return fecha;
}
...
function validarTabla(tablaajax,id){
    let valor = true;
    $(tablaajax+" tbody tr").each(function(){
        fila                 = $(this).find('.tdbusqval').attr('data_id');
        if(fila==id){
            valor=false;
        }               
    });
